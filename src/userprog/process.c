...
start_process(void* file_name_)
{
    ...
    spt_init(&thread_current()->spt); //! spt initialization
...
void process_exit(void)
{
    ...
    spt_hash_destroy(&thread_current()->spt); //! destroy spt
    ...
    uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
    ...
    while (read_bytes > 0 || zero_bytes > 0) {
        ...
        struct frame* kpage = frame_get_page(); //* PROJECT 3
        if (kpage == NULL)
            return false;

        /* Load this page. */
        if (file_read(file, kpage->kpage, page_read_bytes) != (int)page_read_bytes) {
            frame_free_page(kpage->kpage);
            return false;
        }
        memset(kpage->kpage + page_read_bytes, 0, page_zero_bytes);
...
setup_stack(void** esp)
{
    ...
    *esp = PHYS_BASE;
    struct frame* kpage = frame_get_page();
    if (install_page(upage, kpage->kpage, true)) {
        struct spt_entry* spte = malloc(sizeof(struct spt_entry));
        if (spte == NULL)
            return false;
        ...
        if (!insert_spt(&thread_current()->spt, spte)) {
            free(spte);
            return false;
        }
    } else {
...
//* Because of install_page, I moved this function here.
bool load_physmem(struct spt_entry* spte)
{
    struct frame* kpage = frame_get_page();
    ...
    case VM_FILE:
        ...
        if (file_read(spte->file, kpage->kpage, spte->read_bytes) != (off_t)(spte->read_bytes)) {
            frame_free_page(kpage->kpage);
            return false;
        }
        memset(kpage->kpage + spte->read_bytes, 0, spte->zero_bytes);
        break;
    case VM_ANON:
        swap_in(spte->swap_index, kpage->kpage);
        break;
    }

    if (!install_page(spte->vpage, kpage->kpage, spte->writable)) {
        frame_free_page(kpage->kpage);
        return false;
    }
...
{
    struct frame* kpage = frame_get_page();
    struct spt_entry* new_spte = malloc(sizeof *new_spte);

    new_spte->type = VM_ANON;
    new_spte->vpage = pg_round_down(addr);
    new_spte->writable = true;
    new_spte->is_loaded = true;
    insert_spt(&thread_current()->spt, new_spte);
    kpage->spte = new_spte;
    if (!install_page(new_spte->vpage, kpage->kpage, new_spte->writable)) {
        frame_free_page(kpage->kpage);
        free(new_spte);
    }
}